...
- Replace all occurrences of "Py2Cpp" in the relevant CMakeLists.txt with the name of your project
  - Capitalization matters here: `Py2Cpp` means the name of the project, while `py2cpp` is used in file names.
  - Remember to rename the `include/py2cpp` directory to use your project's lowercase name and update all relevant `#include`s accordingly.
...
To collect code coverage information, run CMake with the `-DPY2CPP_ENABLE_COVERAGE=1` option.
...
# run standalone
./build/Py2Cpp --help
# build docs (requires -DPY2CPP_BUILD_DOCS=ON at configure time)
cmake --build build --target GenerateDocs
```

### Static analysis

clang-tidy can be enabled by configuring CMake with `-DPY2CPP_ENABLE_CLANG_TIDY=ON` and building the `clang-tidy` target.
