...
namespace fun {
     ...
     *
     * @tparam Mn The numeric type
     * @param[in] _m First number
     * @param[in] _n Second number
     * @return Mn The greatest common divisor
     */
    ...
    template <typename Z> struct Fraction {
         ...
         * @param[in] den
         */
        CONSTEXPR14 Fraction(Z num, Z den) : _num{std::move(num)}, _den{std::move(den)} {
            this->normalize();
        }
